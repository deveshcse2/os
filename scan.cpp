...
void SCAN(vector<int>& arr, int head, int disk_size, string direction){ int seek_count = 0;
...
for (int i = 0; i < arr.size(); i++) { if (arr[i] < head)
...
int disk_size = 200; string direction = "left";
SCAN(arr, head, disk_size, direction); return 0;
}
